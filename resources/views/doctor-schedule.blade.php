...
@section('content')
    ...
    <div class="container mx-auto px-5 lg:px-20 xl:px-32 py-16">
        <div class="grid justify-center grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5 items-start">
            <!-- Card  -->
            @foreach($doctors as $doctor)
            <div class="card w-full relative">
                <div>
                    <img class="dr__img h-[350px] w-[270px] object-contain rounded-t-xl" src="{{ asset($doctor->image)}}" alt="{{ $doctor->name }}">
                </div>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        const dr_imgs = document.getElementsByClassName('dr__img');

        Array.from(dr_imgs).forEach(function(dr_img) {
            console.log(dr_img.src, 'dr_img4567');

            if (dr_img.src.includes('male-avatar') || dr_img.src.includes('female-avatar')) {
                dr_img.classList.add('!object-cover');
            }
        });
    });
</script>
